...
class AdminCog(commands.Cog):
    ...
    @app_commands.command(name="shop-post-panel", description="Post a persistent Shop Panel with custom embed to a channel")
    @app_commands.checks.has_permissions(administrator=True)
    ...
    async def shop_post_panel(self, interaction: discord.Interaction, category_id: str, channel: discord.TextChannel):
        logger.info(f"Command /shop-post-panel invoked by {interaction.user} for category {category_id} in channel {channel.name}.")
        try:
            # Send modal immediately (category validation happens in modal submit)
            modal = EmbedJsonModal(category_id=category_id, channel=channel)
            await interaction.response.send_modal(modal)
            logger.info(f"Successfully sent EmbedJsonModal to {interaction.user}.")
        except Exception as e:
            logger.error(f"Failed to send modal in /shop-post-panel: {e}", exc_info=True)
            try:
                # The initial response failed, so we must use a followup.
                await interaction.followup.send("An error occurred while trying to open the modal.", ephemeral=True)
            except Exception as followup_e:
                logger.error(f"Failed to send error followup to user: {followup_e}")
